...
def get_alojamientos(db):
    ...
    for r in results:
        ...
        r["servicios_adicionales"] = (
            json.loads(r["servicios_adicionales"]) if r["servicios_adicionales"] else []
        )
    return results
...
def get_alojamiento_detalle(db, alojamiento_id: int):
    ...
    alojamiento["comodidades"] = (
        json.loads(alojamiento["comodidades"]) if alojamiento["comodidades"] else []
    )
    alojamiento["imagenes"] = (
        json.loads(alojamiento["imagenes"]) if alojamiento["imagenes"] else []
    )
    alojamiento["servicios_adicionales"] = (
        json.loads(alojamiento["servicios_adicionales"])
        if alojamiento["servicios_adicionales"]
        else []
    )
    return alojamiento
...
def crear_alojamiento(db, data: AlojamientoCreate):
    cursor = db.cursor()
    sql = (
        "INSERT INTO alojamientos (nombre, estado, capacidad, tipo, descripcion, comodidades, precio_por_noche, imagenes, servicios_adicionales, politicas) "
    ...
    values = (
        ...
        (
            json.dumps(data.servicios_adicionales)
...
def actualizar_alojamiento(db, alojamiento_id: int, data: AlojamientoUpdate):
    ...
    try:
        ...
        raise HTTPException(
            status_code=422, detail=f"Error al actualizar alojamiento: {str(e)}"
        )
