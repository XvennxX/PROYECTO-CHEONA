...
from app.models.alojamiento_model import (
    Alojamiento,
    AlojamientoCreate,
    AlojamientoUpdate,
)
...
def actualizar_alojamiento_put(
    id: int, data: AlojamientoUpdate, db: Session = Depends(get_db)
):
    return alojamiento_service.actualizar_alojamiento(db, id, data)


@router.patch("/{id}", response_model=Alojamiento)
def actualizar_alojamiento_patch(
    id: int, data: AlojamientoUpdate, db: Session = Depends(get_db)
):
    return alojamiento_service.actualizar_alojamiento(db, id, data)
